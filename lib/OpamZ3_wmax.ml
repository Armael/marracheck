...
let name = "builtin-hacked-wmax-z3"
...
let def_criterion ctx opt (preamble, universe, request as cudf) psym
  ...
  in
  let value_f = read_pkg_property_value preamble property in

  let (_, universe, _) = cudf in
  let pkgs = Cudf.fold_packages (fun pkgs pkg ->
    match filter_f pkg with
    | None -> pkgs
    | Some f ->
      match value_f pkg with
      | 0 -> pkgs
      | n -> (f, n) :: pkgs
  ) [] universe in

  if pkgs = [] then None else begin
    let label =
      Z3.Symbol.mk_string ctx @@
      match property with None -> "unknown_prop" | Some s -> s in
    List.map (fun (e, n) ->
      match sign with
      | Plus ->
        Z3.Optimize.add_soft opt e (string_of_int n) label
      | Minus ->
        Z3.Optimize.add_soft opt (Z3.Boolean.mk_not ctx e)
          (string_of_int n) label
    ) pkgs
    |> OpamStd.Option.some
  end
...
let call ~criteria ?timeout (preamble, universe, request as cudf) =
  let (ctx, opt, psym) =
    match !cc with
    | Some (ctx, opt, psym, u) when u = universe (* todo: diffing *) ->
      log "Reusing existing state";
      (ctx, opt, psym)
    | _ ->
      (* try *)
      ...
      let st = (ctx, opt, psym, universe) in
      cc := Some st; (ctx, opt, psym)
  in
  ...
  let expand_constraint (name, constr) =
    ...
    | Some (`Eq, ver) ->
      ...
      OpamStd.Option.map (fun e -> pkg, e) (psym pkg)
    | _ -> assert false
  in
  ...
  let _ =
    ...
    |> List.iter (fun batch ->
      let batch_id = fst (List.hd batch) in
      let batch = List.map snd batch in
      List.iter (fun (pkg, e) ->
        let weight = read_pkg_property_value preamble (Some "version-lag") pkg in
        ignore @@
        Z3.Optimize.add_soft opt e (string_of_int (100 - weight))
          (Z3.Symbol.mk_string  ctx ("to_install_" ^ string_of_int batch_id))
      ) batch;
      ()
    )
  in
